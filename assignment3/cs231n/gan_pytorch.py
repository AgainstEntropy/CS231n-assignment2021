...
def generator_loss(logits_fake):
    ...
    target_fake = torch.ones_like(logits_fake).type(dtype)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss_real = torch.pow(scores_real - 1, 2).mean(dim=0)
    loss_fake = torch.pow(scores_fake, 2).mean(dim=0)
...
def ls_generator_loss(scores_fake):
    ...
    loss = torch.pow(scores_fake - 1, 2).mean(dim=0) / 2
